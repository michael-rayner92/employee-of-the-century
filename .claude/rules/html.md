...
- All `<script>` tags load at the end of `<body>` and their order **is** the dependency graph: `data/*.js` → `utils/utils.js` → `lib/democracy-api.js` → `script.js` → `scripts/*.js`. New files must be slotted into the right layer.